...
// public view
add_action('wp_enqueue_scripts', array(new WPEUC, 'enqueue_public_static'));
add_action('init', array(new WPEUC, 'get_public_view'));

// admin view
if(is_admin()) {
	add_action('admin_enqueue_scripts', array(new WPEUC, 'enqueue_admin_static'));
	add_action( 'admin_menu', array(new WPEUC, 'get_admin_view'));
	add_action('admin_init', array(new WPEUC, 'register_and_build_admin_settings'));
}
